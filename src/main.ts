...
import { GRID_BLACK, normalizeGridColor } from './shared/mapberry'
...
function normalizeScene(input: Partial<MapScene>): MapScene | null {
  ...
  return {
    ...
    gridType: input.gridType === 'square' || input.gridType === 'hex' ? input.gridType : 'none',
    ...
    gridVisible: input.gridVisible !== false && input.gridType !== 'none',
    gridThickness: clamp(finite(input.gridThickness, 0.5), 0.25, 5),
    gridColor: normalizeGridColor(input.gridColor ?? GRID_BLACK),
    ...
    fogOpacity: clamp(finite(input.fogOpacity, 1), 0.1, 1),
